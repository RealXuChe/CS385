...
class Pair<T1, T2> {
  ...
  equals(other: Pair<T1, T2>): boolean { return this.first === other.first && this.second === other.second; }
}
class PairSet<T1, T2> extends Set<Pair<T1, T2>> {
  ...
  has(pair: Pair<T1, T2>): boolean { return Array.from(this).some(p => p.equals(pair)); }
}
...
export default function Home() {
  ...
  const drawGraph = () => {
    let newDot = dotHead;
    const addEdge = (u: string, v: string) => { newDot += u + "--" + v + "; "; }
    const addNode = (u: string) => { newDot += u + "[" + "shape=circle" + "]" + "; "; }
    edges.forEach((e) => { if (e.first == root) addEdge(e.first, e.second); });
    edges.forEach((e) => { if (e.first != root) addEdge(e.first, e.second); });
    nodes.forEach((node) => { addNode(node); });
    nodes.forEach((node) => {
      if (node == root && deg.get(node)! == 1) addEdge(node, 'nullnode[style="invis"]');
      if (node != root && deg.get(node)! == 1) addEdge(node, 'nullnode[style="invis"]'), addEdge(node, 'nullnode[style="invis"]');
      if (node != root && deg.get(node)! == 2) addEdge(node, 'nullnode[style="invis"]');
    });
    newDot += dotTail;
    // console.log(newDot);
    setDot(newDot);
  ...
  const delNode = () => {
    ...
    needToDel.forEach((e) => { actualDelEdge(e) });
  ...
  const delEdge = () => {
    ...
    if (!edges.has(new Pair<string, string>(EdgeToDelU, EdgeToDelV)) && edges.has(new Pair<string, string>(EdgeToDelV, EdgeToDelU)))
      actualDelEdge(new Pair<string, string>(EdgeToDelV, EdgeToDelU));
    else if (edges.has(new Pair<string, string>(EdgeToDelU, EdgeToDelV)) && !edges.has(new Pair<string, string>(EdgeToDelV, EdgeToDelU)))
  ...
  return (
    ...
    >
      <Box mt={2}>
        <TextField type="text" id="nodeToAdd" value={nodeToAdd} onChange={handleOnChangeOfAdd} />
        <Button onClick={addNode}>
          Add a node
        </Button>
      </Box>
      <Box mt={2}>
        <TextField type="text" id="nodeToDel" value={nodeToDel} onChange={handleOnChangeOfDel} />
        <Button onClick={delNode}>
          Delete a node
        </Button>
      </Box>
      <Box mt={2}>
        <TextField type="text" id="edgeToAddU" value={EdgeToAddU} onChange={handleOnChangeOfAddU} />
        <TextField type="text" id="edgeToAddV" value={EdgeToAddV} onChange={handleOnChangeOfAddV} />
        <Button onClick={addEdge}>
          Add an edge
        </Button>
      </Box>
      <Box mt={2}>
        <TextField type="text" id="edgeToDelU" value={EdgeToDelU} onChange={handleOnChangeOfDelU} />
        <TextField type="text" id="edgeToDelV" value={EdgeToDelV} onChange={handleOnChangeOfDelV} />
        <Button onClick={delEdge}>
          Delete an edge
        </Button>
      </Box>
